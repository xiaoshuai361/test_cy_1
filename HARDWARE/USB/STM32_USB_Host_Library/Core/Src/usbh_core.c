                                              ...
                                              uint8_t id), uint8_t id)
{
  ...
  phost->os_event = xQueueCreate(MSGQUEUE_OBJECTS, sizeof(uint32_t));
...
//#endif /* defined (USBH_PROCESS_STACK_SIZE) */
  ...
  #if defined (USBH_PROCESS_STACK_SIZE)
    uint32_t stack_size = USBH_PROCESS_STACK_SIZE;
  #else
    uint32_t stack_size = 8U * configMINIMAL_STACK_SIZE;
  #endif /* defined (USBH_PROCESS_STACK_SIZE) */

//  USBH_Thread_Atrr.priority = USBH_PROCESS_PRIO;
//  phost->thread = osThreadNew(USBH_Process_OS, phost, &USBH_Thread_Atrr);
	xTaskCreate(USBH_Process_OS,"USBH_Queue",stack_size,phost,USBH_PROCESS_PRIO,&phost->thread); 
...
/**
  ...
  * @retval USBH_Status
  */
extern char GamePad_Manufacturer[100] ;
extern char GamePad_iSerialNum[100] ;
static USBH_StatusTypeDef USBH_HandleEnum(USBH_HandleTypeDef *phost)
{
  USBH_StatusTypeDef Status = USBH_BUSY;
  USBH_StatusTypeDef ReqStatus = USBH_BUSY;
	USBH_UsrLog("em:%d",phost->EnumState);
  switch (phost->EnumState)
  {
    case ENUM_IDLE:
      /* Get Device Desc for only 1st 8 bytes : To get EP0 MaxPacketSize */
      ReqStatus = USBH_Get_DevDesc(phost, 64U);
      ...
      else
      {
		  ...
		  if( ++ testtime>=500 ) NVIC_SystemReset();
		  USBH_UsrLog("state:%d",ReqStatus);
        /* .. */
      }
      break;
    ...
    case ENUM_GET_MFC_STRING_DESC:
      if (phost->device.DevDesc.iManufacturer != 0U)
      {
        ...
        if (ReqStatus == USBH_OK)
        {
		  ...
		  memcpy(GamePad_Manufacturer,(char *)(void *)phost->device.Data,strlen((char *)(void *)phost->device.Data));
    ...
    case ENUM_GET_SERIALNUM_STRING_DESC:
      if (phost->device.DevDesc.iSerialNumber != 0U)
      {
        ...
        if (ReqStatus == USBH_OK)
        {
		  ...
		  memcpy(GamePad_iSerialNum,(char *)(void *)phost->device.Data,strlen((char *)(void *)phost->device.Data));
          Status = USBH_OK;
        }
        else if (ReqStatus == USBH_NOT_SUPPORTED)
        {
...
//	vTaskDelay(1000);
  for (;;)
  {
    status = osMessageQueueGet(((USBH_HandleTypeDef *)argument)->os_event,
                               &((USBH_HandleTypeDef *)argument)->os_msg, NULL, portMAX_DELAY);
    if (status == 0)
    {
      USBH_Process((USBH_HandleTypeDef *)argument);
    }
